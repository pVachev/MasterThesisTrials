...
\new{A dimension that this literature leaves largely unexamined is the \emph{funding} of a
sector tilt: which existing holdings are sold to pay for it. The papers above either hold
the sector sleeve alongside a fixed core or fund it uniformly, so the question of whether
the funding source should depend on the character of the tilt does not arise. It matters
here because the two satellite styles play opposite roles. A cyclical tilt is an
amplification of equity risk and is naturally financed by the portfolio as a whole; a
defensive tilt is a substitution away from equity risk, and financing it pro-rata would
liquidate part of the bond block at exactly the moment that block is most valuable. The
present study therefore adopts a style-dependent displacement rule under which cyclical
satellites are funded pro-rata from the core while defensive satellites and gold are funded
from the equity leg alone, so that the bond weight scales only with the cyclical
allocation. This is a portfolio-construction choice rather than an alpha signal, and it
follows the separation advocated by Grinold and Kahn (2000) between the model that ranks
opportunities and the rules that implement them: the displacement rule is applied
identically to every candidate and uses only the static style label of each satellite, so
it never enters the ranking itself.}
