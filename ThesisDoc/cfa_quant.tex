...
\usepackage{tabularx}
\usepackage{xcolor}
%% Drafted text pending PV language review. To accept all and remove the red,
%% change the definition below to:  \newcommand{\new}[1]{#1}
\newcommand{\new}[1]{{\color{red}#1}}
...
Taylor-expanded expected utility function after Jondeau and Rockinger (2006), accommodating
...
protection, all confined to the sizing stage and none visible to selection: a style-dependent
displacement rule funds defensive tilts from the equity leg alone, leaving the bond block intact
in risk-off months; a downside realised-semivariance ratio computed from daily returns sharpens
the bear probability that sizes the satellite sleeve; and a blended bond--equity correlation
sets the internal equity--bond split of the core. At a 30\% satellite sleeve the framework
...
manager, portfolio manager, and trading desk, and to abide by rules and regulations.

Once a standardised approach has been developed for a firm, an operational pipeline follows,
and its scale depends, again, on the size and cost of operations. According to PwC's Asset and
Wealth Management 2025 report, passive AUM is projected to reach USD~70 trillion by 2030 at an
...
\textcolor{red}{[PV: your note asked whether to add more sources here. My view: one more is enough and the trend is already stark. Suggest adding a single fee-compression datapoint (e.g. average expense ratios) and stopping there --- in 5--7 pages this paragraph earns its place by being short and pointed. Tell me if you want me to draft the sentence.]}
...
The main empirical results are reported in \textbf{Table~\ref{tab:performance}}. We contribute
...
structural breaks in sector leadership. Second, allocation decisions are scored rather than
optimised, making the approach computationally manageable within a real operational pipeline.
Third, multiple investor preference types, from mean-variance to mean-variance-kurtosis, are
accommodated within the same framework. Finally, a score improvement floor acts as a hurdle
rate, so the model deviates from the passive core only when the regime signal justifies it.
...
aimed at drawdown protection, all confined to the sizing stage: a style-dependent displacement
rule that funds defensive tilts from equity so the bond block survives risk-off months; a
downside realised-semivariance signal from daily returns that sharpens the bear probability
sizing the sleeve; and a blended bond--equity correlation that sets the internal equity--bond
split of the core. Keeping all three out of selection preserves the two-stage separation: the
regime model decides \emph{what} to hold and \emph{which way} to lean, the scalars only
\emph{how much}.}

\new{[cut: intro paragraph on Ang \& Bekaert / Guidolin \& Timmermann --- duplicated verbatim in Section~2.1, so removed here; the PV note on adding a fourth departure is retained below]}

The present study sits in that tradition but departs from it in three ways: a simpler,
operationally manageable scoring approach rather than full Bellman equation optimisation; a US
sector ETF universe rather than international equity; and rolling window re-estimation as the
mechanism for handling structural breaks, which studies relying on the stable 1954--1999 data
environment did not require.
...
\textcolor{red}{[PV: roadmap to be rewritten once the final section structure is fixed. Holding until the 5--7 page skeleton is agreed --- no point renumbering twice.]}

\clearpage
% ============================================================
\clearpage
\section{Literature Review}
\textcolor{red}{[PV: you are rewriting this section with the volatility and correlation literature. The hybrid-B paragraph is now in the Sector ETF subsection below (with Grinold \& Kahn). For the new material the load-bearing citations are Connolly, Stivers \& Sun (2005) for the armed gate, Moreira \& Muir (2017) and Patton \& Sheppard (2015) for the volatility signal, Baele, Bekaert \& Inghelbrecht (2010) for the correlation horizon, and Cederburg et al. (2020) as the counterpoint.]}
\label{sec:litreview}
% ============================================================

\subsection{Regime Switching in Asset Allocation}

The regime-switching framework originates with Hamilton (1989), whose filtered probability
recursion allows the latent state to be inferred from observed data, producing a probabilistic
assessment of the current regime rather than a binary classification. The first direct
application to portfolio allocation is Ang and Bekaert (2002), who show that correlations
increase substantially during bear regimes, eroding the diversification benefit static
mean-variance analysis predicts. Ang and Bekaert (2004) demonstrate that regime-aware
allocation outperforms a static 60/40 benchmark even after transaction costs, with the key
mechanism being regime persistence: regimes last long enough for the signal to be actionable.
Guidolin and Timmermann (2007) provide the most comprehensive treatment, estimating a four-state
model on US stocks and bonds and solving the full dynamic programming problem; the welfare cost
of ignoring regimes reaches approximately eight cents per dollar at a 60-month horizon under
high risk aversion. A finding directly relevant here is that no VAR terms are required,
suggesting the return distribution itself, rather than predictive variables, carries the
information. The Ang and Timmermann (2012) survey documents that three or more states outperform
two-state models, supporting the specification used here.

\new{[cut: 2.1: Hamilton EM detail, the all-equity welfare-cost figures, G\&T state names and out-of-sample discussion, and the three-contributions paragraph --- the last is stated in the Introduction]}

\subsection{Sector ETF Allocation under Regime Switching}

The application of regime switching to sector-level allocation is a narrower and more recent
strand. Ma, MacLean, Xu, and Zhao (2011) is the closest antecedent, proposing a
regime-dependent factor-risk-neutral strategy for the nine State Street SPDR sector ETFs, the
same universe used here and selected on the same criteria: a single provider, a long trading
history beginning December 1998, and comprehensive coverage of the US market. Their framework
does not include a fixed bond core, does not incorporate higher-moment preferences, and uses an
expanding estimation window; the rolling window adaptation and the core-plus-satellite
architecture are contributions of the present study. Dou, Gallagher, Schneider, and Walter
(2014) provide direct evidence that cross-sector regime-dependent allocation outperforms
cross-regional allocation, reaching an out-of-sample Sharpe ratio of 0.93 against 0.30 for the
world market portfolio, with notably lower beta in bear regimes. Their documentation of lower
cross-sector correlation during bear regimes supports the motivation for the conviction scalar
used here. Jiang, Liu, and Tse (2015) find comparable results on international iShares ETFs
after transaction costs, evidence that ETF-based regime-switching strategies are implementable
at the institutional level.
\new{[cut: 2.2: benchmark-constrained Sharpe figures from Dou et al., and the daily-rebalancing detail from Jiang et al.]}

\textcolor{red}{[PV: the two paragraphs below are mine and you said you want to rewrite them in your own voice --- left untouched for that. They are currently ~250 words; budget ~120.]}

\new{A dimension this literature leaves unexamined is the \emph{funding} of a sector tilt:
which existing holdings are sold to pay for it. It matters because the two satellite styles
play opposite roles. A cyclical tilt amplifies equity risk and is naturally financed by the
portfolio as a whole; a defensive tilt substitutes away from equity risk, and financing it
pro-rata would liquidate part of the bond block at exactly the moment that block is most
valuable. The present study therefore funds cyclical satellites pro-rata from the core and
defensive satellites from the equity leg alone, so the bond weight scales only with the
cyclical allocation. Following the separation advocated by Grinold and Kahn (2000) between the
model that ranks opportunities and the rules that implement them, this is a
portfolio-construction rule rather than an alpha signal: it is applied identically to every
candidate and uses only the static style label of each satellite, so it never enters the
ranking itself.

This funding rule shares its motivation with the two conditioning scalars introduced in
Section~\ref{sec:allocation}. The upside of the regime signal was already established in the
thesis-stage results; what remained was protection against sudden drawdown episodes, and all
three mechanisms are organised around the same bear-regime read. The displacement rule
preserves the bond block structurally whenever the model turns defensive; the volatility
scalar sharpens the bear probability that triggers that rotation; and the correlation scalar
decides how much of the core should be held in bonds once it does. Because the three act on
different parts of the portfolio --- funding, sleeve size, and core composition --- but respond
to the same underlying state, they compound rather than offset, which is why the downside
metrics improve materially while the upside capture is preserved.}

\subsection{Higher-Moment Preferences and Taylor-Expanded Utility}

Scott and Horvath (1980) provide the theoretical foundation: their result that investors prefer
positive skewness and dislike excess kurtosis holds broadly for risk-averse utility functions
rather than specific parametric forms, so the investor types defined here rest on a universal
property of risk-averse behaviour rather than an assumed functional form. Jondeau and Rockinger
(2006) supply the operational implementation, a Taylor expansion of expected utility around the
moments of the return distribution that yields a closed-form scoring function and avoids the
computationally prohibitive co-skewness and co-kurtosis tensors direct optimisation would
require. They find the opportunity cost of using only mean and variance ranges from negligible
in near-normal environments to more than 25 cents per dollar in highly non-normal settings.
Where they evaluate a single combined MVSK investor, we separate MVS and MVK types so the
contribution of each moment can be isolated empirically.

\new{[cut: 2.3: Kim, Fabozzi, Cheridito \& Fox (2014) on robust mean-variance implicitly favouring skewness --- complementary but not load-bearing]}

\subsection{Rolling Window Estimation and Time-Varying Parameters}

The rolling window approach is grounded in the parameter instability literature. Pesaran and
Timmermann (2007) establish that under structural breaks the optimal window trades off bias from
stale data against variance from insufficient observations, and that rolling windows are
preferred when the data-generating process exhibits parameter instability. Hematizadeh,
Tajaddini, and Hallahan (2022) find that even constant-transition-probability HMMs with rolling
estimation capture most of the time-variation benefit of more complex TVTP models, supporting
the simpler constant-$P$ specification used here. Nystrup, Hansen, Madsen, and Lindstr{\"o}m
(2015) show that regime-based allocation with adaptive parameter estimation outperforms static
allocation without requiring return forecasting skill, and their use of a confidence threshold
before changing positions is the conceptual antecedent of the score improvement floor.

\new{[cut: 2.4: the $k=3$ design-tension paragraph. It is a LIMITATION rather than literature --- suggest one sentence in the robustness section instead: $k$ is selected on the full sample, so it is a modelling assumption rather than a fully out-of-sample choice]}

\new{[cut: 2.4: the extended discussion of why re-selecting $k$ each window would be computationally expensive and methodologically inconsistent --- reduced to the caveat]}

\new{\subsection{Volatility and Correlation as Conditioning Signals}

Numerous studies, most notably Moreira and Muir (2017) and Patton and Sheppard (2015), show
empirically that high volatility and volatility spikes tend to be followed by further elevated
volatility, and to be preceded by negative returns. Patton and Sheppard show that this
asymmetry is driven specifically by the downside component of realised variance, which is why
the signal used here is a realised semivariance rather than a total variance.

A broad literature explores the time-varying relation between assets, and in particular the
stock--bond relation. The sign of that relation is not stable: during crises and drawdown
episodes the correlation between broad stocks and bonds flips negative, signifying
flight-to-quality market movements. At the same time, Baele, Bekaert and Inghelbrecht (2010)
find that macroeconomic fundamentals play little role in determining or even explaining
stock--bond correlation. Their study underlines one of the reasons why we chose to keep this
model endogenous and explore only volatility and correlation as potential scaling signal
candidates.

In \textit{Modeling and Forecasting Realized Volatility}, Andersen, Bollerslev, Diebold and
Labys (2003) show that high-frequency data can be aggregated to measure volatility accurately
at lower frequencies. Their work uses exchange rates, but the authors underline the parallels
to other asset classes. When we researched how monthly rebalancing could be influenced by
volatility and correlation signals of higher frequency, a major concern was that daily data
would introduce too much noise into the model. For that reason, we left the daily volatility
and correlation data to work as a separate mechanism in the architecture, measuring the state
of a month rather than triggering trades within it.

Their second main argument is that parametric volatility modelling becomes unwieldy as the
number of assets grows, whereas realised measures remain a model-free and essentially unbiased
estimator of return volatility, and stay tractable in a high-dimensional setting. Our model
explores a large number of candidate combinations at each monthly rebalancing date, so this
tractability is a practical requirement rather than a theoretical nicety. Following Andersen et
al., we build a non-conflicting volatility and correlation mechanism.

Focusing on correlation, Ledoit and Wolf (2004) argue that covariance matrices contain
substantial estimation error, and propose a shrinkage mechanism that rectifies it. The central
idea is that extreme covariance values are pulled towards the centre, much as a winsorised mean
pulls in outliers. We apply the same idea to the estimation error produced by small regimes:
whenever a regime holds fewer than twelve observations inside the current five-year window, its
covariance is replaced by the pooled window covariance, so that the slow correlation layer is
never represented by an unstable estimate. On top of that we blend it with a faster-reacting
correlation layer that further dilutes any erroneous signalling into the scaling mechanism.

\textcolor{red}{[PV: two checks. (i) Your draft said Andersen et al. show ``worse results when
forecasting daily'' --- as I read them, their realised-volatility forecasts beat GARCH at both
one- and ten-day horizons, so I left the claim out. Verify before restoring. (ii) Your ``over
long periods the relation stays low positive'' holds pre-2000 but not 2000--2020, so I cut that
sentence rather than assert it. (iii) I replaced the direct quotation from Andersen et al.\ with
a paraphrase.]}}
...
\textcolor{red}{[PV: you marked this subsection and Estimation for removal in the competition version. Agreed --- both are standard and can be compressed to a short paragraph plus a citation to Hamilton (1989), with the full specification moved to an appendix. Leaving intact for now since volume is not yet the constraint.]}
...
\new{\item Andersen, T.G., Bollerslev, T., Diebold, F.X. and Labys, P. (2003). Modeling and
...
\new{\item Baele, L., Bekaert, G. and Inghelbrecht, K. (2010). The determinants of stock and
...
\new{\item Grinold, R.C. and Kahn, R.N. (2000). \textit{Active Portfolio Management: A
...
\new{\item Ledoit, O. and Wolf, M. (2004). Honey, I shrunk the sample covariance matrix.
...
\new{\item Moreira, A. and Muir, T. (2017). Volatility-managed portfolios.
...
\new{\item Patton, A.J. and Sheppard, K. (2015). Good volatility, bad volatility: signed jumps
