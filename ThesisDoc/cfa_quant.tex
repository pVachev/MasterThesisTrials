...
dominance in the post-2016 period. \new{A score improvement floor acts as an adoption hurdle: the portfolio holds the passive
60/40 core in any month in which no candidate tilt improves on that core by more than the
floor, so the model trades only when the scorecard justifies the cost of trading.
Satellite tilts are funded by a style-dependent displacement rule: cyclical satellites are
funded pro-rata from the whole core, while defensive satellites and gold are funded from the
equity leg alone, so that the bond block is left intact precisely in the months the model
turns defensive. Two conditioning scalars, both confined to the sizing stage and neither
visible to selection, complete the design: a downside realised-semivariance ratio computed
from daily returns sharpens the bear probability that sizes the satellite sleeve, and a
blended bond--equity correlation---combining the regime-mixture estimate with a
horizon-matched realised estimate from daily data---sets the internal equity--bond split of
the core.}
\textcolor{red}{[PV: headline numbers below now use the final grid (hybrid-B, $\kappa=0.2$,
$\omega=0.5$, $\lambda=0.2$, MVS). Confirm you want BOTH anchors in the abstract, or pick one.]}
\new{The framework is reported at two operating points on a single frontier. A risk-managed
configuration (25\% satellite sleeve) achieves a CAGR of 9.05\% against a benchmark of
7.71\%, a Sharpe ratio of 1.033 versus 0.875, a maximum drawdown of $-25.6$\% versus
$-30.6$\%, and a downside deviation of 6.80\% versus 7.11\% --- improving on the benchmark
on every risk-adjusted metric simultaneously. A return-seeking configuration (45\% sleeve)
achieves a CAGR of 9.91\% and a Sharpe ratio of 0.998. At both operating points the strategy
captures more than 100\% of benchmark upside while capturing less than 93\% of its downside.}
\end{abstract}
...
predictive moments of the regime mixture alone. The present paper extends that engine with
three mechanisms aimed specifically at drawdown protection, all confined to the sizing stage
and none visible to selection: a style-dependent displacement rule that funds defensive tilts
from equity so the bond block survives risk-off months; a downside realised-semivariance
signal from daily returns that sharpens the bear probability sizing the satellite sleeve; and
a blended bond--equity correlation, combining the regime-mixture estimate with a
horizon-matched realised estimate, that sets the internal equity--bond split of the core.
Keeping all three out of the selection stage preserves the two-stage separation: the regime
model decides \emph{what} to hold and in \emph{which} direction to lean, while the scalars
decide only \emph{how much}.}
...
horizons, rising above eight cents at 60-month horizons under high risk aversion.
\textcolor{red}{[PV: your note here asked whether to bring in points from above. The three departures are already stated immediately below, so the paragraph does not need them repeated. What is MISSING is the upgrade: the three departures listed are the thesis-era ones. Suggest adding a fourth --- that all signal conditioning is confined to the sizing stage, leaving selection a pure distribution-scoring problem. Confirm and I will write it.]}
The present
...
The two scalars added to the sizing stage rest on a separate empirical literature, which is
reviewed here because it motivates both the choice of estimator and the decision to keep
these signals out of the selection engine.

The volatility side rests on two well-documented regularities. Volatility is strongly
persistent, so that periods of elevated volatility tend to be followed by further elevated
volatility, and it is asymmetric, in that negative returns raise subsequent volatility by
more than positive returns of the same magnitude. Patton and Sheppard (2015) show that this
asymmetry is driven specifically by the downside component of realised variance, which is
what motivates the use of realised semivariance rather than total realised variance as the
conditioning signal in this study. Moreira and Muir (2017) establish the portfolio
implication: because volatility is persistent but is not compensated one-for-one in expected
returns at the monthly horizon, scaling exposure down when volatility is high improves
risk-adjusted performance. Their result also carries a caution that shapes the design here.
Elevated volatility forecasts variance rather than the mean, which argues for rotating the
sleeve toward defensive holdings rather than exiting risk altogether.

The correlation side rests on the time-varying nature of the stock--bond relation. The sign
of that correlation is not stable across decades: it was persistently positive before 2000,
persistently negative through the 2000--2020 period, and turned positive again during the
2022 rate shock. The negative-correlation state is the flight-to-quality regime in which
bonds hedge equity drawdowns, and it is precisely the state in which a bond allocation earns
its place. Baele, Bekaert and Inghelbrecht (2010) find that macroeconomic fundamentals
explain remarkably little of the variation in stock--bond comovement, with liquidity and
uncertainty proxies doing more of the work. That finding supports the decision taken here to
keep the model endogenous: rather than conditioning on macroeconomic state variables, this
study uses only volatility and correlation measured from returns themselves as candidate
scaling signals.

The choice to build these signals from daily data, while rebalancing monthly, follows
Andersen, Bollerslev, Diebold and Labys (2003). Their central result is that realised
measures constructed by aggregating high-frequency returns provide a model-free and
essentially unbiased estimate of the underlying volatility, and that forecasts built on such
measures outperform conventional parametric alternatives. Two features of their argument
matter here. First, the accuracy gain comes from using higher-frequency data to \emph{measure}
a lower-frequency quantity, which is exactly the role the daily series plays in this study:
it measures the volatility and correlation state of a month, and never triggers an
intra-month trade. Second, they emphasise that realised measures remain tractable in higher
dimensions, where parametric multivariate volatility models become unwieldy. Since the
allocation layer evaluates a large number of candidate combinations at each monthly
rebalancing date, this tractability is a practical requirement rather than a theoretical
nicety.

\textcolor{red}{[PV: two claims from your draft I could not verify and did not want to assert
on your behalf. (i) You wrote that Andersen et al. show ``worse results when trying to
forecast daily''. As I read the paper, their realised-volatility forecasts \emph{outperform}
GARCH-type models at both one-day and ten-day horizons, so I have not reproduced that claim
above. Please check before restoring it. (ii) You wrote that the stock--bond relation ``over
long periods remains in the low positive values''. That is true of the pre-2000 sample but not
of 2000--2020, so I have written the sign as regime-dependent instead. Adjust if you meant
something narrower.]}

Finally, the treatment of estimation error in the correlation layer follows Ledoit and Wolf
(2004), who show that sample covariance matrices estimated from short samples contain
substantial estimation error and propose shrinking extreme entries toward a structured target.
The same concern applies with particular force here, because the slow correlation estimate is
conditioned on regimes and the thinnest regime in a sixty-month window is frequently populated
by only a handful of observations. Two safeguards follow from this. Any regime with fewer than
twelve observations inside the current estimation window has its covariance replaced by the
pooled window covariance, so that a sparsely populated state cannot drive the core split. In
addition, the resulting estimate is blended with a faster realised correlation computed from
daily data, which further dilutes the influence of any single unstable regime estimate on the
scaling mechanism.
\textcolor{red}{[PV: the Ledoit--Wolf link is an analogy rather than an implementation --- we
do not compute a shrinkage intensity. I have phrased it as ``follows'' and ``the same
concern'', which is defensible. If you would rather claim less, say ``in the same spirit
as''.]}}
