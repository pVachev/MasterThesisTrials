...
The primary configuration achieves a CAGR of 8.06--8.16\% against a benchmark of 7.71\%,
Sharpe ratios of 0.923--0.932 versus 0.875, and maximum drawdown improvements of approximately
380 basis points, while simultaneously beating the benchmark on downside deviation across all
investor types.
\end{abstract}
...
We contribute to the regime-switching and portfolio allocation literature in several ways.
...
optimisation problem, making the approach computationally tractable within a real operational
...
operationally tractable scoring approach rather than full Bellman equation optimisation. We
...
full Bellman equation optimisation with a computationally tractable scoring approach, making
the framework operational within a wealth management pipeline. Third, and most importantly,
...
dislike excess kurtosis) holds for all utility functions satisfying DARA, not just specific
...
(XAU) is included as a tenth satellite candidate. All sector ETF and gold return series were
...
\textbf{Regime} & \textbf{Asset}
  ...
  & \textbf{Exc.\ Kurt.} & \textbf{Min} & \textbf{Max} \\
\midrule
\multicolumn{8}{l}{\footnotesize\textit{All figures in \% per month; skewness and excess kurtosis are dimensionless.}} \\[2pt]
\midrule
\multirow{2}{*}{\shortstack[l]{\textbf{Regime 0 --- Bear}\\[1pt]\footnotesize\textit{N\,=\,20 (6.2\%)}}}
  & S\&P 500 TR  & $-$2.916 & 7.949 & \phantom{$-$}0.115 & $-$0.972 & $-$18.466 & \phantom{$-$}9.130 \\
  & LT Bond      & $-$0.523 & 3.443 & \phantom{$-$}0.772 & $-$0.174 & $-$4.993  & \phantom{$-$}7.657 \\[6pt]
\midrule
\multirow{2}{*}{\shortstack[l]{\textbf{Regime 1 --- Transitional}\\[1pt]\footnotesize\textit{N\,=\,141 (43.4\%)}}}
  & S\&P 500 TR  & \phantom{$-$}0.101 & 5.038 & $-$0.191 & $-$0.385 & $-$13.303 & \phantom{$-$}12.062 \\
  & LT Bond      & \phantom{$-$}0.573 & 1.647 & \phantom{$-$}0.027 & $-$0.438 & $-$3.519  & \phantom{$-$}4.747 \\[6pt]
\midrule
\multirow{2}{*}{\shortstack[l]{\textbf{Regime 2 --- Bull}\\[1pt]\footnotesize\textit{N\,=\,164 (50.5\%)}}}
  & S\&P 500 TR  & \phantom{$-$}1.301 & 2.612 & $-$0.236 & $-$0.100 & $-$6.139  & \phantom{$-$}8.300 \\
  & LT Bond      & $-$0.116 & 1.686 & $-$0.415 & \phantom{$-$}0.765 & $-$5.915  & \phantom{$-$}4.001 \\[6pt]
\midrule
\multirow{2}{*}{\shortstack[l]{\textbf{Full Sample}\\[1pt]\footnotesize\textit{N\,=\,325 (100\%)}}}
  & S\&P 500 TR  & \phantom{$-$}0.521 & 4.384 & $-$0.661 & \phantom{$-$}1.080 & $-$18.466 & \phantom{$-$}12.062 \\
  & LT Bond      & \phantom{$-$}0.158 & 1.854 & $-$0.044 & \phantom{$-$}0.861 & $-$5.915  & \phantom{$-$}7.657 \\
\bottomrule
\end{tabular}
\begin{minipage}{\linewidth}
\vspace{4pt}\footnotesize
\textit{Notes:} Excess log returns are $\log(1+r_t) - \log(1+r_{f,t})$ where $r_{f,t}$ is the one-month T-bill rate.
Regime assignments use the Viterbi hard-state sequence from the best-seed three-state HMM (Model A, without gold).
Skewness and excess kurtosis follow Fisher's definition.
S\&P 500 TR = \texttt{\^{}SP500TR}; LT Bond = \texttt{LT09TRUU} Bloomberg Long Treasury index (20$+$ year).
...
\begin{placeholder}{FIGURE 1 — Insert here}
Smoothed regime probability chart (3 stacked panels, \texttt{sharex=True}), SP500TR and
LT09TRUU, 1999--2026 --- showing 3 regimes with key crisis episodes annotated.
\texttt{fig1\_regime\_probabilities.png}
\end{placeholder}
...
The primary output of the HMM for allocation purposes is the one-step-ahead predictive
probability vector:
\begin{equation}
...
\begin{placeholder}{TABLE 1 — Insert here}
HMM regime-conditional moments --- mean, std dev, skewness, kurtosis for SP500TR and
LT09TRUU by regime; transition matrix $\mathbf{P}$; expected duration per regime;
\% of observations. From \texttt{hmm\_regime\_results\_monthly.xlsx}.
\end{placeholder}
...
of the utility curve within the mean-variance framework. Using proof by contradiction within
an expected utility framework and under the assumption of decreasing absolute risk aversion
(DARA), they determine the sign of preference for higher moments \textit{a priori}, without
requiring any specific parametric form for the utility function.
...
large number of assets while remaining computationally tractable, which is an important
...
Specifically, at the unconditional moments of the benchmark 60/40 portfolio over the full
...
The satellite sleeve introduces up to two sector or commodity tilts that displace only the
S\&P~500 component of the core. With a maximum satellite weight of 20\%, the S\&P~500
allocation ranges from 60\% when no tilt is active to a minimum of 40\% when the full sleeve
is deployed. LT09TRUU remains at 40\% in all configurations. No leverage is introduced at
any point.
...
The primary configuration uses a 20\% maximum satellite sleeve, a score improvement floor
...
rebalance decisions. Table~\ref{tab:performance} presents the performance across investor
...
\toprule
\textbf{Configuration} & \textbf{CAGR} & \textbf{vs BM}
  & \textbf{Sharpe} & \textbf{Max DD} & \textbf{Down Dev}
  & \textbf{Sortino} & \textbf{Calmar} & \textbf{Omega} & \textbf{NONE\%} \\
\midrule
Benchmark (60/40)        & 7.71\% & ---      & 0.875 & $-$30.61\% & 7.11\% & 1.104 & 0.252 & 1.935 & --- \\
\midrule
20\%/0.001 / MV          & 8.06\% & +0.35\%  & 0.923 & $-$26.78\% & 7.05\% & 1.159 & 0.301 & 1.977 & 33\% \\
20\%/0.001 / MVS\_cons   & 8.16\% & +0.45\%  & 0.932 & $-$26.78\% & 7.06\% & 1.171 & 0.305 & 1.989 & 32\% \\
20\%/0.001 / MVS         & 8.14\% & +0.43\%  & 0.932 & $-$26.89\% & 7.02\% & 1.175 & 0.303 & 1.989 & 33\% \\
20\%/0.001 / MVK         & 8.13\% & +0.42\%  & 0.931 & $-$27.00\% & 6.98\% & 1.180 & 0.301 & 1.983 & 32\% \\
\midrule
30\%/0.001 / MVS$^{*}$   & 8.37\% & +0.66\%  & 0.938 & $-$25.39\% & 7.26\% & 1.167 & 0.330 & 2.000 & 8\%  \\
30\%/0.002 / MVK         & 8.14\% & +0.43\%  & 0.932 & $-$25.62\% & 7.07\% & 1.165 & 0.318 & 1.989 & 57\% \\
35\%/0.002 / MVK$^\dagger$& 8.40\% & +0.69\% & 0.950 & $-$25.28\% & 7.20\% & 1.180 & 0.333 & 2.012 & 48\% \\
\bottomrule
\end{tabular}
\begin{minipage}{\linewidth}
\vspace{4pt}\footnotesize
\textit{Notes:} All configurations use the rolling 60-month window, 60/40 core,
equity-only satellite displacement, sectors and gold, 5\,bp transaction costs, 2004--2026.
$^*$ Downside deviation exceeds benchmark (7.11\%).
$^\dagger$ Best Sharpe ratio across all configurations.
NONE\% = proportion of months holding passive 60/40 core.
\end{minipage}
\end{table}

All four investor types in the primary configuration beat the benchmark simultaneously on
CAGR, Sharpe ratio, maximum drawdown, downside deviation, Sortino ratio, Calmar ratio, and
Omega ratio. This five-metric simultaneous outperformance is the defining characteristic of
the primary result and the basis for selecting the 20\% sleeve with floor\,=\,0.001 as the
final configuration.
...
\begin{figure}[H]
  ...
  \caption{Cumulative wealth for all four investor types versus the 60/40 benchmark
           (February 2004 -- January 2026, \$1 invested). Shaded regions mark crisis
           episodes.}
  \label{fig:cumwealth}
\end{figure}
...
The outperformance is not uniformly distributed across time. In the 2004--2009 period, the
model generates average active alpha of $+0.125\%$ per month, driven by correctly identifying
XLE as the dominant satellite during the commodity supercycle. In the 2010--2015 period, the
model generates modest positive alpha of $+0.010\%$ per month. In the 2016--2026 period, the
MV investor produces a small alpha drag of $-0.008\%$ per active month, which the MVS and MVK
investors eliminate, recording $+0.007\%$ and $+0.014\%$ respectively.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{figures/fig3_satellite_weights_grid}
  \caption{Monthly satellite allocation weights by sector (MVS\_cons, primary configuration
           2004--2026). Bold frames: XLE (energy cycle 2004--09), XLK (technology cycle
           2016--26), XAU (gold, crisis periods).}
...
size above 20\%. The NONE rate collapses from 33\% to 3\%. The 20\% sleeve with
floor\,=\,0.001 is the only configuration that beats the benchmark on all five metrics
simultaneously. The full sensitivity results are presented in Appendix~\ref{sec:appendix}.
...
\begin{factcheck}
The gold section references ``a speculative rally driven by retail investors'' and a
``US-Israeli war with Iran causing a 10\% gold drop.'' These specific claims require
factual verification: the 2025 gold rally was primarily driven by central bank and
institutional demand (World Gold Council 2025), not retail. The geopolitical event
description and the 10\% drop figure need to be checked and corrected before submission.
\end{factcheck}
...
This is a deliberate trade-off for operational tractability.
...
\begin{placeholder}{APPENDIX TABLE — Insert here}
Full sensitivity table from \texttt{backtest\_full\_ratio\_table\_with\_none.html} ---
all sleeve sizes (20\%--50\%), both floors (0.001, 0.002), all investor types
(MV, MVS\_cons, MVS, MVK), all metrics (CAGR, Sharpe, MaxDD, DwnDev, Sortino, Calmar,
Omega, NONE\%).
\end{placeholder}
